...
class User {
  ...
  public async login(ctx: any, next: Function): Promise<void | Error> {
    try {
      ...
      let access_token: string = await tokenUser.create_token(user_id + tokenTime);
      // 查询
  ...
  // 注册用户
  public async register(ctx: any, next: Function): Promise<void | Error> {
    try {
      let {
        user_id = "",
        user_name = "",
  ...
  // 查询用户
  public async findUserById(ctx: any, next: Function): Promise<void | Error> {
    try {
  ...
  // 注销用户
  public async logout(ctx: any, next: Function): Promise<void | Error> {
    try {
      let res = null;
   ...
   * 获取用户个人信息
   */
  public async getUserInfo(ctx: any, next: Function): Promise<void | Error> {
    try {
      let res = "";
      responseUser.setData(res);
      ctx.body = responseUser;
